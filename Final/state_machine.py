...
from lights import (
    ...
    STATE_WAITING, STATE_STOPPED, STATE_ARRIVED,
)

# Physical constants
CPS_TO_MPS = (1.0 / (720 * 4)) * ((13.0 * 19.0) / (70.0 * 37.0)) \
             * 2.0 * np.pi * (0.066 / 2)


class StateMachineConfig:
    # Speed
    V_REF_NAVIGATE  = 0.3    # m/s normal cruising speed
    V_REF_AVOID     = 0.15   # m/s during obstacle avoidance
...
class StateMachine:
    ...
    def update(self, detection: dict, nav_result: dict,
        ...
        """
        One FSM tick. Returns throttle, steering, state.
        ...
        # Convert motor tach to wheel speed (m/s)
        current_speed = float(sensor_data['motor_speed']) * CPS_TO_MPS
        ...
        if self.state == STATE_ARRIVED:
            ...
            # Timeout: if waited too long, try navigate anyway
        ...
        if self.state == STATE_NAVIGATING:
            ...
            steering = nav_steer  # heading PID from Navigator

        elif self.state == STATE_AVOIDING:
            v_ref    = self.cfg.V_REF_AVOID
            throttle = self._speed_pid.update(r=v_ref, y=current_speed, dt=dt)
            # Steer away from obstacle (add avoidance offset to navigator)
        ...
        return {
            ...
            'v_ref':    self.cfg.V_REF_NAVIGATE if self.state == STATE_NAVIGATING
